...
public class PersistentQueueDefinitionTests {
    ...
    public void testConfigureSupplier() {
        var queue = new PersistentQueueDefinition(EXCHANGE_NAME, QUEUE_NAME, Duration.ofSeconds(3)).setMaxAttempts(2).setConcurrency(1);
    ...
    public void testConfigureSupplierWithSingleActiveConsumer() {
        var queue = new PersistentQueueDefinition(EXCHANGE_NAME, QUEUE_NAME, Duration.ofSeconds(3)).setSingleActiveConsumer().setMaxAttempts(2).setConcurrency(1);
        var producer = new ProducerDefinition(SUPPLIER_NAME, queue);
    ...
    public void testConfigureProcessor() {
        var queue = new PersistentQueueDefinition(EXCHANGE_NAME, QUEUE_NAME, Duration.ofMinutes(2)).setMaxAttempts(2).setConcurrency(5);
        ...
        assertThat(props.size()).isEqualTo(7);
    ...
    public void testConfigureProcessorWithSingleActiveConsumer() {
        var queue = new PersistentQueueDefinition(EXCHANGE_NAME, QUEUE_NAME, Duration.ofMinutes(2)).setSingleActiveConsumer().setConcurrency(6).setMaxAttempts(2);
        var consumer = new ConsumerDefinition(PROCESSOR_NAME, queue);
